import { BodyPresetKey } from '../data/solar-system'
  ...
  return (
    <div className="bg-gray-950 border-b border-gray-800 select-none">
      <div className="flex flex-wrap items-center gap-2 px-4 py-2 text-xs">
        ...
        <button
          onClick={onReset}
          title="Reset to the real solar system"
          className="px-2.5 py-1 rounded bg-gray-700 hover:bg-gray-600 text-gray-200 transition-colors"
        >
          ↺ Reset Solar System
        </button>
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded flex-shrink-0 transition-colors font-medium ${
            addingBody ? 'bg-blue-600 text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'
          }`}
      ...
      {addingBody && (
        <div className="flex items-center gap-2 px-4 pb-2 text-xs border-t border-gray-800/60">
          ...
          <span className="ml-2 mt-1.5 text-gray-500 italic">
